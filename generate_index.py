...
class ContentRenderer:
    ...
    def _render_pdf(self, path: str) -> str:
        """将 PDF 渲染为 canvas 页面，高清 + 懒加载，兼容手机。"""
        with open(path, 'rb') as f:
            b64 = base64.b64encode(f.read()).decode('ascii')
        return (
            '<div id="pdf-container" style="width:100%;max-width:900px;margin:0 auto;"></div>\n'
            '<p id="pdf-loading" style="text-align:center;color:#888;padding:40px;">加载 PDF 中...</p>\n'
            ...
            '          renderPage(num);\n'
            '          if(num+1<=pdf.numPages)renderPage(num+1);\n'
            '          observer.unobserve(e.target);\n'
            '        }\n'
            '      });\n'
            '    },{rootMargin:"200px"});\n'
            '    for(var p=3;p<=pdf.numPages;p++){\n'
            '      observer.observe(document.getElementById("pdf-page-"+p));\n'
            '    }\n'
            '  });\n'
            '}\n'
            '\n'
            f'fetch("data:application/pdf;base64,{b64}").then(function(r){{return r.arrayBuffer();}}).then(function(buf){{\n'
            '  initPdf(new Uint8Array(buf));\n'
            '});\n'
            '})();\n'
            '</script>'
        )
