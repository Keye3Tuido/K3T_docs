...
class ContentRenderer:
    ...
    def _render_pdf(self, path: str) -> str:
        ...
        return (
            ...
            '    });\n'
            '  }\n'
            '  pdfjsLib.getDocument({data:bytes,cMapUrl:"https://cdn.jsdelivr.net/npm/pdfjs-dist@3.11.174/cmaps/",cMapPacked:true}).promise.then(function(pdf){\n'
...
#page-loading .bar {{ height: 100%; background: #4a90d9; border-radius: 2px;
  width: 0%; transition: width 0.3s; }}
</style>
{extra_head}
</head>
<body oncontextmenu="return false;">
<div id="page-loading"><span id="load-text">加载中 0%</span><div class="bar-wrap"><div class="bar" id="load-bar"></div></div></div>
<script>
(function(){{
  ...
  var timer=setInterval(function(){{
    var nodes=document.body.getElementsByTagName("*").length;
    // 用节点数估算，每 100 个节点约 5%，上限 90%
    var est=Math.min(90, Math.round(nodes/20));
    if(est>pct)pct=est;
    bar.style.width=pct+"%";
    txt.textContent="加载中 "+pct+"%";
  }},200);
  // readyState 变化
  ...
  window.__loadTimer=timer;
  window.__loadSetPct=function(p){{pct=p;bar.style.width=p+"%";txt.textContent="加载中 "+p+"%";}};
}})();
</script>
{body}
<script>
...
(function(){{
  ...
  if(txt)txt.textContent="加载中 100%";
  setTimeout(function(){{
